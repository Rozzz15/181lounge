"use client";

import { cn } from "@/lib/utils";
import { forwardRef, useRef, useCallback, useEffect } from "react";
import { gsap } from "gsap";

export type SocialItem = {
  id: string;
  url: string;
  icon: React.ReactNode;
  label: string;
};

export interface IdentityCardProps extends React.HTMLAttributes<HTMLDivElement> {
  fullName: string;
  place: string;
  about: string;
  avatarUrl: string;
  avatarText: string;
  icon?: React.ReactNode;
  scheme?: "plain" | "accented";
  socials?: SocialItem[];
  displayAvatar?: boolean;
  titleCss?: React.CSSProperties;
  cardCss?: React.CSSProperties;
  descClass?: string;
  bioClass?: string;
  footerClass?: string;
}

export const IdentityCardBody = forwardRef<HTMLDivElement, IdentityCardProps>(
  (
    {
      fullName,
      place,
      about,
      avatarUrl,
      avatarText,
      icon,
      scheme = "plain",
      socials = [],
      displayAvatar = true,
      titleCss,
      cardCss,
      descClass,
      bioClass,
      footerClass,
      className,
      ...rest
    },
    ref
  ) => {
    const isAccent = scheme === "accented";

    return (
      <div
        ref={ref}
        style={cardCss}
        className={cn(
          "flex flex-col rounded-3xl p-8 h-full",
          isAccent
            ? "text-[var(--on-accent-foreground)]"
            : "bg-[#F3F0E8] text-[#44362A]",
          className
        )}
        {...rest}
      >
        {/* Header */}
        <div className="p-0">
          <div className={cn(!displayAvatar && "invisible")}>
            {icon ? (
              <div 
                className="h-16 w-16 rounded-full bg-[#525A40]/10 flex items-center justify-center ring-2 ring-offset-4 ring-offset-[#F3F0E8]"
                style={
                  {
                    "--tw-ring-color": "var(--accent-color)",
                    color: isAccent ? "var(--on-accent-foreground)" : "#525A40",
                  } as React.CSSProperties
                }
              >
                {icon}
              </div>
            ) : avatarUrl ? (
              <div 
                className="h-16 w-16 rounded-full overflow-hidden ring-2 ring-offset-4 ring-offset-[#F3F0E8]"
                style={
                  {
                    "--tw-ring-color": "var(--accent-color)",
                  } as React.CSSProperties
                }
              >
                <img src={avatarUrl} alt={avatarText} className="w-full h-full object-cover" />
              </div>
            ) : (
              <div 
                className="h-16 w-16 rounded-full bg-[#525A40]/10 flex items-center justify-center ring-2 ring-offset-4 ring-offset-[#F3F0E8] text-[#525A40] font-bold text-xl"
                style={
                  {
                    "--tw-ring-color": "var(--accent-color)",
                  } as React.CSSProperties
                }
              >
                {avatarText}
              </div>
            )}
          </div>
          <p
            className={cn(
              "pt-6 text-left text-sm",
              !isAccent && "text-[#7a756d]",
              descClass
            )}
            style={isAccent ? { color: "var(--on-accent-muted-foreground)" } : {}}
          >
            {place}
          </p>
          <h3
            className={cn(
              "text-3xl text-left font-heading font-bold mt-1",
              !isAccent && "text-[#44362A]"
            )}
            style={{
              ...(isAccent ? { color: "var(--on-accent-foreground)" } : {}),
              ...titleCss,
            }}
          >
            {fullName}
          </h3>
        </div>

        {/* Content */}
        <div className="mt-6 flex-grow p-0">
          <p
            className={cn(
              "text-base leading-relaxed text-left",
              !isAccent && "text-[#44362A]/80",
              bioClass
            )}
            style={isAccent ? { opacity: 0.9 } : {}}
          >
            {about}
          </p>
        </div>

        {/* Footer */}
        {socials.length > 0 && (
          <div className={cn("mt-6 p-0", footerClass)}>
            <div
              className={cn(
                "flex items-center gap-4",
                !isAccent && "text-[#7a756d]"
              )}
              style={
                isAccent
                  ? { color: "var(--on-accent-muted-foreground)" }
                  : undefined
              }
            >
              {socials.map((s) => (
                <a
                  key={s.id}
                  href={s.url}
                  aria-label={s.label}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={cn(
                    "transition-opacity",
                    isAccent ? "hover:opacity-75" : "hover:text-[#44362A]"
                  )}
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>
        )}
      </div>
    );
  }
);
IdentityCardBody.displayName = "IdentityCardBody";

// ------------------ Animated Container ------------------

export interface RevealCardProps extends React.HTMLAttributes<HTMLDivElement> {
  base: React.ReactNode;
  overlay: React.ReactNode;
  accent?: string;
  textOnAccent?: string;
  mutedOnAccent?: string;
}

export const RevealCardContainer = forwardRef<HTMLDivElement, RevealCardProps>(
  (
    {
      base,
      overlay,
      accent = "#525A40",
      textOnAccent = "#fff",
      mutedOnAccent = "rgba(255,255,255,0.8)",
      className,
      ...rest
    },
    ref
  ) => {
    const holderRef = useRef<HTMLDivElement>(null);
    const overlayRef = useRef<HTMLDivElement>(null);

    const assignRef = useCallback(
      (el: HTMLDivElement | null) => {
        holderRef.current = el;
        if (typeof ref === "function") ref(el);
        else if (ref) (ref as React.MutableRefObject<HTMLDivElement | null>).current = el;
      },
      [ref]
    );

    const startClip = "circle(50px at 64px 64px)";
    const expandClip = "circle(160% at 64px 64px)";

    // Set initial clip path on mount
    useEffect(() => {
      if (overlayRef.current) {
        gsap.set(overlayRef.current, { clipPath: startClip });
      }
    }, []);

    const reveal = () => {
      if (overlayRef.current) {
        gsap.to(overlayRef.current, {
          clipPath: expandClip,
          duration: 0.8,
          ease: "expo.inOut",
        });
      }
    };
    
    const conceal = () => {
      if (overlayRef.current) {
        gsap.to(overlayRef.current, {
          clipPath: startClip,
          duration: 1,
          ease: "expo.out",
        });
      }
    };

    return (
      <div
        ref={assignRef}
        onMouseEnter={reveal}
        onMouseLeave={conceal}
        style={
          {
            "--accent-color": accent,
            "--on-accent-foreground": textOnAccent,
            "--on-accent-muted-foreground": mutedOnAccent,
            borderColor: "var(--accent-color)",
          } as React.CSSProperties
        }
        className={cn(
          "relative w-[300px] overflow-hidden rounded-3xl border-2 bg-white shadow-md cursor-pointer",
          className
        )}
        {...rest}
      >
        <div>{base}</div>
        <div
          ref={overlayRef}
          className={cn("absolute inset-0 h-full w-full bg-[#44362A]")}
        >
          {overlay}
        </div>
      </div>
    );
  }
);
RevealCardContainer.displayName = "RevealCardContainer";
